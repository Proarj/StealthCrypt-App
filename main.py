...
def embed_btn():
    ...
    if get_error("embed") == 0:
        ...
        threading.Thread(target=STG._merge_txt, args=(IMAGE_PATH,TEXT_PATH,DEST_PATH,KEY)).start()
...
def extract_btn():
    ...
    if get_error("extract") == 0:
        threading.Thread(target=STG._unmerge_txt, args=(IMAGE_PATH, DEST_PATH, KEY)).start()

def decrypt_btn():
    """
    This function starts the decryption process.
    """
    if get_error("decrypt") == 0:
        threading.Thread(target=decrypt_text, args=(TEXT_PATH, KEY)).start()

def decrypt_text(text_path, key):
    """
    This function decrypts the text using the provided key and saves the decrypted text to a new text file.
    """
    encrypted_text = fileHandler.read(text_path)
    decrypted_text = STG._decrypt_text(encrypted_text, key)

    # Find the available filename for the decrypted text file
    filename = "decrypted_text"
    index = 0
    while os.path.exists(os.path.join(DEST_PATH, f"{filename}_{index}.txt")):
        index += 1

    # Save the decrypted text to a new text file
    with open(os.path.join(DEST_PATH, f"{filename}_{index}.txt"), "w") as file:
        file.write(decrypted_text)

    messagebox.showinfo("Decryption Complete", "Decryption is completed!")
...
# GUI - TKINTER
window = Tk()
window.title("StealthCrypt app") # title of the window
