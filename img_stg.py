...
class ImgStg():
    ...
    def _merge_txt(self, img_p, txt_p, dest_p, key, showInfo=True):
        ...
        encrypted_text = self._encrypt_text(txt, key)
        img_w, img_h = img.size[0], img.size[1]
        map = img.load()
        txt_bits = self._bytes_to_bin(encrypted_text) 
    ...
    def _decrypt_text(self, text, key):
        fernet = Fernet(key)
        decrypted_text = fernet.decrypt(text.encode('utf-8'))
